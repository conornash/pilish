...
To pick the command per project, set =pilish-executable-function= to a
function of the session directory.  It returns a list shaped like
=pilish-executable=, or nil to use the default:

#+begin_src emacs-lisp
;; Run a project-local launcher when one exists:
(setopt pilish-executable-function
        (lambda (dir)
          (let ((local (expand-file-name "bin/pi" dir)))
            (and (file-executable-p local) (list local)))))
#+end_src
