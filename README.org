...
While streaming or compacting, five minutes without stdout from Pi changes
only the existing input activity status, for example to
=thinking (no output 5m)=, using Emacs's =warning= face.  The phase stays
visible.  Idle and sending sessions are not monitored; hiding the input
also hides the warning.  It never aborts, polls Pi, or changes session state.
Hover for help, or use =M-x pilish-abort= (normally =C-c C-k=) in that session
if you choose to stop and discard queued continuations.

=pilish-session-inactivity-timeout= defaults to =300= seconds.  Use a positive
number to change the threshold, or =nil= to disable the warning:

#+begin_src emacs-lisp
(setopt pilish-session-inactivity-timeout nil)
#+end_src

Changes apply at the next header refresh; reenabling retains the existing
output age.  Any nonempty parent stdout counts, including partial or malformed
output, ordinary replies, and repeated tool progress; stderr does not.
Silence is not proof of a hang: quiet tools and healthy subagents may produce
no parent output, while one sibling's output keeps the shared clock fresh.
Timing is approximate wall time, not an awake-time deadline: suspend, clock
changes, or a blocked Emacs can advance or delay the warning.  Active sessions
refresh the input header once a second when Emacs can service its timers.
...
Press =w= on one strict file target at point to copy an =@path:line=
reference for Pi prompts.  The path is spelled exactly as =@= completion
would insert it: project-relative inside the session directory, otherwise
absolute, with paths containing spaces taking a quoted =@"…"= form.  A known
physical line — a line range uses its first line — appends =:LINE=; targets
without a location copy the path alone.  Paste the reference into a prompt
to point Pi at that file and line.
...
Sessions currently open in a live Pilish process in this Emacs get a ●
marker; it reflects only the frontend's known process state and never a
system process scan, and browsing the disk archive works with no live
process at all.  Press =g= to refresh markers.
...
| =w=              | chat    | 📋 Copy =@path:line= file reference          |
...
| w     | copy @path:line reference at point      |
